...
PROMPT_OPTIONAL() {
	...
	OPTIONAL=""
        ...
        while [[ "$OPTIONAL" != "Y" && "$OPTIONAL" != "N" && "$OPTIONAL" != "y" && "$OPTIONAL" != "n" ]]; do
		read -p "$TEXT [Y/N] " OPTIONAL
	done
...
PROMPT_DESKTOP_XIVQL()
{
    ...
    if [[ "$(find "$HOME/.local/share/applications" -maxdepth 1 -iname 'ffxiv-run-xivql.desktop' -print -quit 2> /dev/null | wc -l)" -gt 0 ]]; then
        echo "Desktop entries have already been created"
        CONTINUE="n"
    fi

    while [[ "$CONTINUE" != "Y" && "$CONTINUE" != "N" && "$CONTINUE" != "y" && "$CONTINUE" != "n" ]]; do
        read -p "Would you like to create desktop entries for XIVQL? [Y/N] " CONTINUE
    done
...
if [[ "$CONTINUE" == "N" || "$CONTINUE" == "n" ]]; then
        ...
        "[Desktop Entry]" \
        "Name=FFXIV (QL) & ACT" \
        "Exec=$HOME/bin/ffxiv-run-xivql-both.sh" \
        "Icon=$HOME/.local/share/icons/hicolor/256x256/apps/act_ffxiv.png" \
        "Type=Application" \
        "Terminal=False" \
        "Categories=Game;" > $HOME/.local/share/applications/ffxiv-run-xivql-both.desktop
        printf '%s\n' \
        "[Desktop Entry]" \
        "Name=Final Fantasy XIV (QL)"\
        "Exec=$HOME/bin/ffxiv-run-xivql.sh" \
        "Icon=$FFXIV_ICON" \
        "Type=Application" \
        "Terminal=False" \
        "Categories=Game;" > $HOME/.local/share/applications/ffxiv-run-xivql.desktop
        echo "Desktop entries have been created at $HOME/.local/share/applications/"
	fi
}
